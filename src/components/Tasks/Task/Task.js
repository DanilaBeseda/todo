...
export const Task = ({ list, onRemoveTask, onEditTask }) => {
      ...
      <>
         {list.tasks.map(task => (
            <div className={classes.item} key={task.id}>
               ...
               <input readOnly value={task.text} onClick={() => onEditTask(list.id, task.id)} />
               {!isLoading
                  ? <div className={classes.btns}>
                     ...
                     <button className={classes.remove} onClick={() => removeTask(list.id, task.id)}>
